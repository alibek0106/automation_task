...
|------|------------|---------|
| Classes | PascalCase | `LoginPage`, `ProductsPage` |
...
grep -r "async.*login\|async.*signup" tests/pages/
```
...
|-------|----------|---------|
| Single test only | Inside the test function | `const SEARCH_TERM = 'blue top';` |
| Single test file | Top of `test.describe` block | `const MIN_PASSWORD_LENGTH = 8;` |
...
const password = testDataGenerator.randomPassword(8);
await steps.verifyMessage('Your account has been created!');

// ✅ GOOD: Named constants explain the meaning
const MIN_PASSWORD_LENGTH = 8;
const ACCOUNT_CREATED_MESSAGE = 'Your account has been created!';

const password = testDataGenerator.randomPassword(MIN_PASSWORD_LENGTH);
await steps.verifyMessage(ACCOUNT_CREATED_MESSAGE);
```
...
async openPage(): Promise<void> {
    await this.page.goto(getEnvironment().baseUrl);
}

// ✅ GOOD: Steps call Page Object navigate()
async openPage(): Promise<void> {
    await this.automationExerciseLandingPage.navigate();
}
```
