...
export class AutomationExerciseLoginPage extends BasePage {
    ...
    constructor(page: Page) {
        ...
        this.emailField = page.locator("[data-qa='login-email']").describe("Email input field");
        this.passwordField = page.locator("[data-qa='login-password']").describe("Password input field");
...
- **Decoupled from Secrets**: Credentials are retrieved in the *test*, not hidden in the Steps.

**See:** `docs/patterns/step-definition.md` for complete Steps pattern

### 6. Test Implementation (Anti-Duplication)
- **Location**: `tests/specs/` (or `tests/`)
...
export class UserApiSteps {
    @step('Create user via API using {0}')
    async createUser(name: string, email: string): Promise<User> {
        const response = await this.apiClient.post<User>('/createAccount', { name, email });
...
test('edit user profile', async ({ userApiSteps, profileSteps }) => {
    const user = await userApiSteps.createUser('John', 'john@test.com'); // Fast API setup
    await profileSteps.navigateToProfile(user.id);                         // UI test
