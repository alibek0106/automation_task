  ...
  () => {
    test("should verify quantity updates and total recalculations", async ({
      ...
      await test.step(`Add ${productName} with Quantity 1`, async () => {
        await cartSteps.openAndAddProductWithQuantity(productName, 1);
      });

      await test.step("Verify initial state in Cart", async () => {
        await productsPage.navigateToCart();

        const qty = await cartSteps.getProductQuantity(productName);
        const total = await cartSteps.getProductTotal(productName);

        expect(qty, "Initial quantity should be 1").toBe(1);
        expect(total, "Initial subtotal should match unit price").toBe(
          pricePerUnit,
        );
      });
      ...
      await test.step("Increase Quantity to 5", async () => {
        await cartSteps.openAndAddProductWithQuantity(productName, 4);
      });
      ...
      await test.step("Verify calculations for Quantity 5", async () => {
        ...
        const subtotal = await cartSteps.getProductTotal(productName);
        expect(subtotal, "Subtotal should be Price * 5").toBe(pricePerUnit * 5);

        const cartTotal = await cartSteps.getCalculatedTotal();
      ...
      await test.step("Decrease Quantity to 2 (Re-add workflow)", async () => {
        ...
        await expect(
          cartPage.getProductRow(productName),
          "Product row should be removed",
        ).toHaveCount(0);

        await cartSteps.openAndAddProductWithQuantity(productName, 2);
      });
      ...
      await test.step("Add second product and verify mixed totals", async () => {
        await cartSteps.openAndAddProductWithQuantity(product2, 3);
        await productsPage.navigateToCart();

        // Verify Product 1 (Still 2)
        const qty1 = await cartSteps.getProductQuantity(productName);
        expect(qty1, "Product 1 quantity should be 2").toBe(2);

        // Verify Product 2 (New 3)
        const qty2 = await cartSteps.getProductQuantity(product2);
        expect(qty2, "Product 2 quantity should be 3").toBe(3);

        // Verify Cart Total
        const expectedTotal = pricePerUnit * 2 + price2 * 3;
        const actualTotal = await cartSteps.getCalculatedTotal();
        expect(
          actualTotal,
          `Cart total should match (${pricePerUnit}*2 + ${price2}*3)`,
        ).toBe(expectedTotal);
      });
    });
  },
